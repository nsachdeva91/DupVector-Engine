...
if option == "View Clusters":
    ...
    if video_path and os.path.isfile(video_path):
        ...
        first_match_fig = None
        first_match_lang_block = ""
        ...
        for rank, (idx, sim) in enumerate(zip(top_k_idx, top_k_sims), start=1):
            ...
            if not first_match_shown and normalized_label in ["video", "video+text", "text"]:
                if os.path.isfile(video_path) and os.path.isfile(full_match_path):
                    if has_audio_stream(video_path) and has_audio_stream(full_match_path):
                        first_match_fig = plot_energy_comparison(video_path, full_match_path)

                        text_a, lang_a = get_transcription_and_language(video_path)
                        text_b, lang_b = get_transcription_and_language(full_match_path)

                        first_match_lang_block = (
                            f"<div style='margin-top:20px'><b>🈶 Language Analysis:</b><br/>"
                            f"Uploaded video: <code>{lang_a}</code><br/>"
                            f"Matched video: <code>{lang_b}</code><br/>"
                        )
                        if lang_a != lang_b:
                            first_match_lang_block += "<span style='color:red'>⚠️ Language mismatch detected.</span>"
                        first_match_lang_block += "</div>"
                    else:
                        st.info("📭 No audio stream detected — skipping audio energy and language analysis.")
                else:
                    st.warning("❗ One or both video files are missing.")
        ...
        if first_match_fig:
            st.markdown("### 🎧 Audio Energy Comparison (Top Match)")
            st.pyplot(first_match_fig)
        if first_match_lang_block:
            st.markdown(first_match_lang_block, unsafe_allow_html=True)

        st.subheader("📊 Video Categorization")
        if distinct:
            st.write("### Distinct Videos (Low similarity)")
            st.dataframe(pd.DataFrame(distinct, columns=df.columns))
        if variants:
            st.write("### Variant Videos (Moderate similarity)")
            st.dataframe(pd.DataFrame(variants, columns=df.columns))
        if duplicates:
            st.write("### Duplicate Videos (High similarity)")
            st.dataframe(pd.DataFrame(duplicates, columns=df.columns))
